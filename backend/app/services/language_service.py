# Try to import SarvamAI; fall back to a stub if unavailable
try:
...
def translate_to_language(english_script, target_language, api_key, mode="code-mixed"):
    ...
    # If Sarvam SDK isn't available, return original text as a safe fallback
    if not _SARVAM_AVAILABLE:
        # Minimal fallback: no translation performed
...
def _translate_text(text, target_language_code, api_key, mode="code-mixed"):
    """Helper function to translate text using SarvamAI if available; otherwise pass-through."""
    if not _SARVAM_AVAILABLE:
        return text
